...
async def remove_application_for_user(bot, member, raid_id):
    queries = []
    queries.append(bot.database.execute(REMOVE_APPLICATION_FOR_USER_BY_ID, member.id))
    queries.append(bot.database.execute(REDUCE_APPLICANT_COUNT_BY_RAID_ID, raid_id))
...
async def set_recent_participation(bot, user_id):
    queries = []
    queries.append(bot.database.execute(DELETE_RECENT_PARTICIPATION_RECORD, int(user_id)))
    queries.append(bot.database.execute(UDPATE_RECENT_PARTICIPATION, int(user_id), datetime.now()))
    await bot.database.batch(queries)
