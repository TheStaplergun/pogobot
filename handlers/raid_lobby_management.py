...
async def extend_duration_of_lobby(bot, ctx):
    ...
    view = bot.extend_lobby_view(bot) if new_extension_time > 1 else None
    await RLH.update_raid_removal_and_lobby_removal_times(bot, lobby_data.get("raid_message_id"), time_to_remove=new_delete_time, lobby_id=lobby_data.get("lobby_channel_id"), view=None)
